...
class Sync():
    ...
    _table_name = 'dbur_det_categorias_personas'
    _table_pk_name = 'id_det_categorias_persona'

    def _get_service_data(self):
        try:
            data = requests.post(self._api_url + 'urecognition_services/get_det_categorias_personas',
    ...
    def _delete(self, id):
        try:
            tabla = Table(self._table_name, self._metadata, autoload=True, autoload_with=self._engine)
            statement = tabla.delete().where(tabla.columns.id_det_categorias_persona == id)
        except Exception as e:
    ...
    def _update(self, item):
        try:
            ...
            statement = update(tabla).values(
                    id_categoria = item['id_categoria'],
                    id_persona = item['id_persona']
                ).where(tabla.columns.id_det_categorias_persona == id)
        except Exception as e:
            self.logger.error("{0}. No se pudo preparar el statement: {1}".format(inspect.stack()[0][3], e))
            return False
        result = self._connection.execute(statement)
